...
fn test_git_head_race_condition() {
    ...
    let output = work_dir.run_jj(["op", "log", "-T", "description"]);
    let divergent_count = output
        .stdout
        .raw()
        .lines()
        .filter(|line| line.contains("reconcile divergent operations"))
        .count();

    if divergent_count > 0 {
        eprintln!("Found {divergent_count} divergent operations:");
        eprintln!("{}", output.stdout.raw());
        panic!("Race condition detected: {divergent_count} divergent operations found");
    }
}
